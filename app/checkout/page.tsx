...
import { ChevronRight, CreditCard, CheckCircle, Lock, Truck, ArrowLeft } from "lucide-react"
...
// ─── Shipping form ────────────────────────────────────────
function ShippingForm({ onNext }: { onNext: (data: Record<string, string>) => void }) {
...
export default function CheckoutPage(): React.ReactElement {
  ...
  function handleShippingNext(data: Record<string, string>) {
        ...
        {step === "confirmed" ? (
          ...
          <div className="flex flex-col lg:flex-row gap-10 items-start">
            <div className="flex-1 min-w-0">
              ...
              {step === "shipping" && <ShippingForm onNext={(d) => handleShippingNext(d)} />}
              {step === "payment"  && <PaymentForm  onNext={() => setStep("review")}  onBack={() => setStep("shipping")} />}
              {step === "review"   && <ReviewStep   shippingData={shippingData} onConfirm={(num) => { setConfirmedOrder(num); setStep("confirmed") }} onBack={() => setStep("payment")} />}
            </div>
            <OrderAside />
          </div>
        )}
