...
export default function AdminShippingPage(): React.ReactElement {
  ...
  async function patchOption(id: number, patch: Partial<ShippingOption>) {
    setError("")
    try {
      const updated = await adminFetch<ShippingOption>(`/vendors/me/shipping/${id}/`, {
        method: "PATCH",
      ...
      {showForm && (
        <form onSubmit={createOption} className="bg-dp-bg-surface border border-dp-border rounded-sm p-5 grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <label className="flex items-center gap-2 self-end">
            <input type="checkbox" checked={form.is_express} onChange={(e) => setForm((f) => ({ ...f, is_express: e.target.checked }))} className="w-3.5 h-3.5 accent-dp-accent-cta" />
            <span className="text-[11px] font-semibold text-dp-text-secondary">Express</span>
          </label>
          ...
          <table className="w-full min-w-[720px]">
            <thead className="border-b border-dp-border">
              <tr className="text-[10px] font-bold uppercase tracking-widest text-dp-text-tertiary">
                ...
                <th className="text-center px-4 py-3">Min Days</th>
                <th className="text-center px-4 py-3">Max Days</th>
                <th className="text-center px-4 py-3">Express</th>
              ...
              {options.map((o) => (
                <tr key={o.id} className="hover:bg-dp-bg-elevated">
                  <td className="px-4 py-3">
                    <select value={o.market} onChange={(e) => void patchOption(o.id, { market: e.target.value })}
                      className="px-2 py-1 bg-dp-bg-elevated border border-dp-border rounded-sm text-[12px]">
                      {MARKETS.map((m) => <option key={m.value} value={m.value}>{m.label}</option>)}
                    </select>
                  </td>
                  <td className="px-4 py-3">
                    <input value={o.label}
                      ...
                      onBlur={(e) => void patchOption(o.id, { label: e.target.value })}
                      className="w-full px-2 py-1 bg-dp-bg-elevated border border-dp-border rounded-sm text-[12px] focus:outline-none focus:border-dp-border-hover" />
                  </td>
                  <td className="px-4 py-3 text-right">
                    <input type="number" min="0" step="0.01" value={o.price}
                      onChange={(e) => setOptions((prev) => prev.map((x) => x.id === o.id ? { ...x, price: e.target.value } : x))}
                      onBlur={(e) => void patchOption(o.id, { price: e.target.value })}
                      className="w-20 px-2 py-1 bg-dp-bg-elevated border border-dp-border rounded-sm text-[12px] text-right focus:outline-none focus:border-dp-border-hover" />
                  </td>
                  <td className="px-4 py-3 text-center">
                    <input type="number" min="1" value={o.est_days_min}
                      onChange={(e) => setOptions((prev) => prev.map((x) => x.id === o.id ? { ...x, est_days_min: parseInt(e.target.value) || 1 } : x))}
                      onBlur={(e) => void patchOption(o.id, { est_days_min: parseInt(e.target.value) || 1 })}
                      className="w-14 px-2 py-1 bg-dp-bg-elevated border border-dp-border rounded-sm text-[12px] text-center focus:outline-none focus:border-dp-border-hover" />
                  </td>
                  <td className="px-4 py-3 text-center">
                    <input type="number" min="1" value={o.est_days_max}
                      onChange={(e) => setOptions((prev) => prev.map((x) => x.id === o.id ? { ...x, est_days_max: parseInt(e.target.value) || 1 } : x))}
                      onBlur={(e) => void patchOption(o.id, { est_days_max: parseInt(e.target.value) || 1 })}
                      className="w-14 px-2 py-1 bg-dp-bg-elevated border border-dp-border rounded-sm text-[12px] text-center focus:outline-none focus:border-dp-border-hover" />
                  </td>
                  <td className="px-4 py-3 text-center">
                    <input type="checkbox" checked={o.is_express}
                      onChange={() => void patchOption(o.id, { is_express: !o.is_express })}
                      className="w-3.5 h-3.5 accent-dp-accent-cta" />
                  </td>
                  <td className="px-4 py-3 text-center">
                    <button type="button" onClick={() => void patchOption(o.id, { is_active: !o.is_active })} className={`text-[10px] font-bold uppercase tracking-widest px-2 py-0.5 border rounded-sm ${o.is_active ? "text-dp-success bg-dp-success/10 border-dp-success/30" : "text-dp-text-tertiary border-dp-border"}`}>
