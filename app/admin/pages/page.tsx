...
type PageSection = { id: number; page: string; section_key: string; title: string; content: Record<string, unknown>; content_ka?: Record<string, unknown> | null; sort_order: number; is_active: boolean }
...
function SectionEditor({ section, onSaved }: { section: PageSection; onSaved: () => void }) {
  const [values, setValues] = useState<[Record<string, unknown>, Record<string, unknown>]>([section.content, (section.content_ka ?? emptyLike(section.content)) as Record<string, unknown>])
  const [saving, setSaving] = useState(false)
  useEffect(() => setValues([section.content, (section.content_ka ?? emptyLike(section.content)) as Record<string, unknown>]), [section])
  async function save() { setSaving(true); await adminFetch(`/admin/page-sections/${section.id}/`, { method: "PATCH", body: JSON.stringify({ content: values[0], content_ka: values[1] }) }); setSaving(false); onSaved() }
  return <div className="flex flex-col gap-4"><div className="flex items-center justify-between"><h3 className="font-display text-lg text-dp-text-primary">{section.title}</h3><span className="text-[10px] uppercase tracking-widest text-dp-text-tertiary">{section.section_key}</span></div><div className="grid grid-cols-1 xl:grid-cols-2 gap-4">{["English", "Georgian"].map((lang, i) => <div key={lang} className="border border-dp-border rounded-sm p-4"><h4 className="text-[11px] font-bold uppercase tracking-widest mb-4">{lang}</h4><StructuredFields value={values[i]} onChange={(path, next) => setValues((all) => all.map((v, index) => index === i ? setNested(v, path, next) as Record<string, unknown> : v) as [Record<string, unknown>, Record<string, unknown>])} /></div>)}</div><button type="button" onClick={() => void save()} disabled={saving} className="self-start flex items-center gap-2 px-5 py-2 bg-dp-accent-cta text-white text-[12px] font-bold uppercase tracking-widest rounded-sm disabled:opacity-50"><Save size={14} /> {saving ? "Saving..." : "Save section"}</button></div>
}
