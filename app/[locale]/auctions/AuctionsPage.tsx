...
function AuctionSubscribeBox() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setStatus("loading")
    try {
      const data = await apiFetch<{ detail: string }>("/auctions/subscribe/", {
        method: "POST",
  ...
  return (
    <div className="bg-dp-bg-surface border border-dp-border rounded-sm p-5">
      <div className="flex items-center gap-2 mb-2">
        <Bell size={16} className="text-dp-accent-cta" />
        <h3 className="font-display text-lg text-dp-text-primary">Get Notified</h3>
      </div>
      <p className="text-[12px] text-dp-text-secondary mb-4">
        Enter your email to be notified when new auctions go live.
      </p>
      {status === "done" ? (
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-2.5">
          <input
            ...
            className="w-full px-3 py-2 text-sm rounded-sm bg-dp-bg-elevated border border-dp-border text-dp-text-primary placeholder:text-dp-text-tertiary focus:outline-none focus:ring-1 focus:ring-dp-accent-cta"
          />
          <button
            type="submit"
            disabled={status === "loading"}
            className="w-full py-2 text-sm font-semibold bg-dp-accent-cta text-white rounded-sm hover:opacity-90 disabled:opacity-50 transition-opacity"
          >
        ...
        {loading ? (
          ...
          <div className="flex flex-col xl:flex-row gap-8">
            ...
            <div className="xl:w-72 flex flex-col gap-6 shrink-0">
              ...
              {/* Auction notification subscribe */}
              <AuctionSubscribeBox />
            </div>
          </div>
        )}
